...
def process_all_wavs_in_folder():
    ...
    max_value = float('-inf') 
    min_value = float('-inf') 
    for key, melspec in melspecs_dict.items():
        current_max = np.max(melspec)
        current_min = np.min(melspec)

        if current_max > max_value:
            max_value = current_max
        if current_min < current_min:
            min_value = current_min
    print("current_max melspec", current_max )
    print("min_value melspec", min_value )
