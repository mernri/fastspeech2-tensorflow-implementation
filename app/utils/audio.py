...
def get_melspecs_from_audio_files(audio_files_dict, sr=SAMPLE_RATE, n_fft=N_FFT, hop_length=HOP_LENGTH, n_mels=N_MELS):
    ...
    for sequence_id, audio_path in audio_files_dict.items():
        melspecs_dict[sequence_id] = waveform_to_mel_spectrogram_from_spectrum(
...
def pad_single_melspec(mel, max_length):
    ...
    if padding > 0:
        mel = np.pad(mel, ((0, 0), (0, padding)), mode='constant',
                     constant_values=MEL_SPEC_PADDING_VALUE)
    return mel
...
def get_padded_melspecs_dict(melspecs_dict):
    ...
    padded_mels = [pad_single_melspec(mel, max_length)
                   for mel in melspecs_lists]

    padded_melspecs_dict = {key: value for key,
                            value in zip(melspecs_dict.keys(), padded_mels)}
...
def get_melspec_by_sequence_id(sequence_id):
    ...
    if not os.path.exists(file_path):
        raise FileNotFoundError(
            f"File {file_name} not found in {PATH_PADDED_MELSPECS}.")
    return np.load(file_path, allow_pickle=True)


def listen_to_audio(melspec):
    hifi_gan = HIFIGAN.from_hparams(
        source="speechbrain/tts-hifigan-ljspeech", savedir="tmpdir")
    ...
    if isinstance(melspec, tf.Tensor):
        ...
        raise ValueError(
            "Unsupported melspec type. Expected tf.Tensor, torch.Tensor, or numpy array.")
    ...
    torchaudio.save('example_own_model.wav', waveforms.squeeze(1), 22050)
