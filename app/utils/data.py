...
def get_audio_files_from_ljspeech(wavs_directory_path=PATH_LJ_AUDIOS):
    ...
    wav_files = [file for file in os.listdir(
        wavs_directory_path) if file.endswith('.wav')]

    audio_files_dict = {}
    for wav_file in wav_files:
        sequence_id = wav_file.strip('.wav')
        audio_files_dict[sequence_id] = os.path.join(
            wavs_directory_path, wav_file)
...
def get_ljspeech_transcripts_from_metadata(transcripts_csv_path=PATH_LJ_CSV):
    ...
    with open(transcripts_csv_path, 'r', encoding='utf-8') as file:
        ...
        for line in reader:
            ...
            transcription = line[2] if len(
                line) > 2 and line[2].strip() else line[1]
...
def get_audio_duration_from_directory(audio_files_path=PATH_LJ_AUDIOS):
    ...
    durations_dict = {sequence_id: get_audio_duration(
        file_path) for sequence_id, file_path in audio_files.items()}
