    ...
    if isinstance(input_data, dict):
        ...
        for filename in os.listdir(input_data):
            if filename.endswith('.npy'):
                arr = np.load(os.path.join(input_data, filename),allow_pickle=True)
...
def count_oov_in_transcripts(transcripts_dir, lexicon_file):
    # Compte le nombre de mots que MFA n'a pas trouvé dans son lexique
    with open(lexicon_file, 'r') as f:
        lexicon = set(line.split()[0] for line in f)
    ...
    for filename in os.listdir(transcripts_dir):
        if filename.endswith('.txt'): 
            with open(os.path.join(transcripts_dir, filename), 'r') as f:
                content = f.read()
                words = content.split() 
                oov_words = [word for word in words if word not in lexicon]
                oov_counts[filename] = len(oov_words)
